...
// The bundled FastAPI backend lives on the same origin under /api, so the
// chat works out of the box without any environment variables.
...
export const StreamProvider: React.FC<{ children: ReactNode }> = ({
  ...
  // falling back to the same-origin bundled backend.
  const finalApiUrl = apiUrl || envApiUrl || getDefaultApiUrl();
  const finalAssistantId =
    assistantId || envAssistantId || DEFAULT_ASSISTANT_ID;
  const finalAuthScheme = authScheme || envAuthScheme || "";

  // The same-origin default URL is only computable in the browser, so wait
  // for the client mount before deciding between form and chat (avoids a
  // hydration mismatch).
